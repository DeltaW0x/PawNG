...
Renderer::Renderer(Window &window, GPUDevice &device,
  ...
  int2 windowSize = m_window.GetSize();
  ...
  m_colorTargetInfo.type = SDL_GPU_TEXTURETYPE_2D;
                       m_colorTargetInfo.format = m_colorTargetFmt;
                       m_colorTargetInfo.usage = SDL_GPU_TEXTUREUSAGE_COLOR_TARGET |
                                SDL_GPU_TEXTUREUSAGE_SAMPLER;
                       m_colorTargetInfo.width = m_lastWidth;
                       m_colorTargetInfo.height = m_lastHeight;
                       m_colorTargetInfo.layer_count_or_depth = 1;
                       m_colorTargetInfo.num_levels = 1;
                       m_colorTargetInfo.sample_count = SDL_GPU_SAMPLECOUNT_1;

  m_depthTargetInfo.type = SDL_GPU_TEXTURETYPE_2D;
                       m_depthTargetInfo.format = m_depthTargetFmt;
                       m_depthTargetInfo.usage = SDL_GPU_TEXTUREUSAGE_DEPTH_STENCIL_TARGET;
                       m_depthTargetInfo.width = m_lastWidth;
                       m_depthTargetInfo.height = m_lastHeight;
                       m_depthTargetInfo.layer_count_or_depth = 1;
                       m_depthTargetInfo.num_levels = 1;
                       m_depthTargetInfo.sample_count = SDL_GPU_SAMPLECOUNT_1;

  m_finalBlitInfo = {.source = {.mip_level = 0,
...
SDL_GPUTextureFormat Renderer::QueryDepthStencilFmt() {
  ...
  if (m_device.SupportsTextureFormat(SDL_GPU_TEXTUREFORMAT_D32_FLOAT_S8_UINT,
                                     type, usage)) {
    return SDL_GPU_TEXTUREFORMAT_D32_FLOAT_S8_UINT;
  }
  if (m_device.SupportsTextureFormat(SDL_GPU_TEXTUREFORMAT_D24_UNORM_S8_UINT,
                                     type, usage)) {
...
Texture &Renderer::GetColorTarget() { return m_colorTarget; }

Texture &Renderer::GetDepthTarget() { return m_depthTarget; }

void Renderer::PresentFrame() {
  Texture texture;
  if (!m_mainCmd.WaitAndAcquireSwapchainTexture(m_window, texture)) {
    spdlog::critical("Failed to acquire swapchain texture: {0}",
                     SDL_GetError());
  ...
  if (texture) {
    ...
    m_finalBlitInfo.source.texture =
        static_cast<SDL_GPUTexture *>(m_colorTarget);
    m_finalBlitInfo.source.w = m_lastWidth;
    m_finalBlitInfo.source.h = m_lastHeight;
    m_finalBlitInfo.destination.texture =
